...
import NetworkTransmissionChart, {
  ChartData,
} from './NetworkTransmissionChart.vue';
    ...
    const interval = setInterval(() => {
      clients.nodeClient.getStatus(
        ...
        (err: Error, status: Status) => {
          ...
          const txAvgObservations = txObservations.slice(
            Math.max(txObservations.length - 10, 0)
          );
          const rxAvgObservations = rxObservations.slice(
            Math.max(rxObservations.length - 10, 0)
          );
