      ...
      <q-card-section class="q-pa-md row">
        <div class="column col-6">
          ...
          <CopyableField
            ...
            :value="nodeDetails.getPrimaryEndpoint()"
          />
          <CopyableField
            title="Mesh IPv4 Address"
            :value="nodeDetails.getPrivateIpv4()"
          />
          <CopyableField
            title="Mesh IPv6 Address"
            :value="nodeDetails.getPrivateIpv6()"
          />
          <CopyableField
            title="Zone Awareness ID"
            :value="nodeDetails.getZoneAwarenessId()"
          />
        </div>
        <div class="column col-6">
          ...
          <CopyableField
            title="Public Key"
            :value="nodeDetails.getPublicKey()"
          />
        </div>
      </q-card-section>
    </q-card>
...
import {
  ...
  MeshNode,
  MeshGraph,
} from '@buf/tinyzimmer_webmesh-api.grpc_web/v1/mesh_pb';
...
export default defineComponent({
  ...
  computed: {
    showDetails(): boolean {
      return this.detailsHovering || this.detailsSelected;
    },
  },
  setup() {
    ...
    function getNodeDetails(id: string): Promise<MeshNode> {
      return new Promise((resolve, reject) => {
        ...
        req.setId(id);
        clients.meshClient.getNode(
          req,
          clients.rpcMetadata,
          (err: Error, res: MeshNode) => {
            if (err) {
              reject(err);
            } else {
              resolve(res);
    ...
    function getNetworkGraph(): Promise<{ data: Data; options: Options }> {
      loading.value = true;
      return new Promise((resolve, reject) => {
        clients.meshClient.getMeshGraph(
          new Empty(),
          clients.rpcMetadata,
          (err: Error, res: MeshGraph) => {
            if (err) {
              reject(err);
            } else {
