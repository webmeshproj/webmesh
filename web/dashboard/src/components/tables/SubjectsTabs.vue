...
import { Subject, SubjectType } from '@buf/tinyzimmer_webmesh-api.grpc_web/v1/rbac_pb';
...
export default defineComponent({
    ...
    computed: {
        ...
        groups(): string[] {
            const out: string[] = [];
            this.subjects.forEach((s) => {
                if (s.getType() == SubjectType.SUBJECT_GROUP || s.getType() == SubjectType.SUBJECT_ALL) {
                    out.push(s.getName());
                }
            })
            return out;
