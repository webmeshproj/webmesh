          ...
          <q-td colspan="100%">
            <div class="row">
              <div class="text-caption col-12">
                <CopyableField
                  title="Public Key"
                  :value="props.row.getPublicKey()"
                />
              </div>
              <div class="text-caption col-12">
                <strong>WireGuard Endpoints:</strong>
                {{ props.row.getWireguardEndpointsList().join(',') || 'N/A' }}
              </div>
              <div class="text-caption" v-if="props.row.getZoneAwarenessId()">
                <strong>Zone Awareness ID:</strong>
                <i>{{ props.row.getZoneAwarenessId() }}</i>
              </div>
            </div>
          </q-td>
        </q-tr>
...
async function listNodes(): Promise<MeshNode[]> {
  return new Promise((resolve, reject) => {
    clients.meshClient.listNodes(
      new Empty(),
      clients.rpcMetadata,
      (err: Error, res: NodeList) => {
        if (err) {
          reject(err);
          return;
        }
