...
func (g *grpcTransport) Dial(ctx context.Context, address string) (conn *grpc.ClientConn, err error) {
	if g.MaxRetries == 0 {
		g.MaxRetries = 1
	}
	for i := 0; i < g.MaxRetries; i++ {
		conn, err = grpc.DialContext(ctx, address, g.Credentials...)
		if err == nil {
			return
		}
	}
