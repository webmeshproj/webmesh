...
// newSecurity creates a new security protocol.
func newSecurity(id protocol.ID, host host.Host, psk pnet.PSK, privkey crypto.PrivKey, rt *Transport) (*SecureTransport, error) {
...
func (st *SecureTransport) NewSecureConn(ctx context.Context, insecure net.Conn, rpeer peer.ID) (*SecureConn, error) {
	...
	err = st.rt.iface.PutPeer(context.WithLogger(ctx, log), &peer)
	if err != nil {
