...
func newWebmeshTransport(log *slog.Logger, tu transport.Upgrader, host host.Host, key crypto.PrivKey, psk pnet.PSK, gater connmgr.ConnectionGater, rcmgr network.ResourceManager) (WebmeshTransport, error) {
	...
	rt.sec, err = newSecurity(wmproto.SecurityID, host, psk, key, &rt)
	if err != nil {
		return nil, fmt.Errorf("failed to create security transport: %w", err)
	}

	err = host.Peerstore().AddProtocols(rt.peerID, wmproto.SecurityID)
	if err != nil {
		return nil, fmt.Errorf("failed to add security protocol to peerstore: %w", err)
	}
