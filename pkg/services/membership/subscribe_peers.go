...
func (s *Server) SubscribePeers(req *v1.SubscribePeersRequest, stream v1.Membership_SubscribePeersServer) error {
	...
	notify := func(_, _ string) {
		...
		if err != nil {
			log.Error("failed to send wireguard peers", "error", err)
			return
		}
	}
...
func listDNSServers(ctx context.Context, st storage.Storage, peerID string) ([]string, error) {
	var servers []string
...
func listICEServers(ctx context.Context, st storage.Storage, peerID string) ([]string, error) {
	var servers []string
