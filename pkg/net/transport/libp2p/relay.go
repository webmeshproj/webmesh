...
func newUDPRelayWithHostAndCloseFunc(logCtx context.Context, host Host, opts UDPRelayOptions, closef func() error) (*UDPRelay, error) {
	...
	host.Host().SetStreamHandler(UDPRelayProtocolFor(opts.LocalPubKey.WireGuardKey().String()), func(s network.Stream) {
		...
		if !key.Equals(opts.RemotePubKey) {
			log.Error("Peer public key does not match expected public key", "peer", info, "expected", opts.RemotePubKey, "actual", key)
			return
		}
		defer s.Close()
		defer rxrelay.Close()
	...
	go func() {
		defer host.Host().RemoveStreamHandler(UDPRelayProtocolFor(opts.LocalPubKey.WireGuardKey().String()))
		defer close(closec)
		defer close(errs)
		...
		LoopPeers:
			for {
				...
				case peer, ok := <-peerChan:
					...
					stream, err := host.Host().NewStream(connectCtx, peer.ID, UDPRelayProtocolFor(opts.RemotePubKey.WireGuardKey().String()))
					connectCancel()
					if err != nil {
