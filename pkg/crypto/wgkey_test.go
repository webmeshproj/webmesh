...
func TestEncodedKeysRoundTrip(t *testing.T) {
	...
	encodedPriv := privkey.String()
	decodedPriv, err := DecodePrivateKeyV2(encodedPriv)
	if err != nil {
		t.Fatal(err)
	}
	...
	encodedPub := pubkey.String()
	decodedPub, err := DecodePublicKeyV2(encodedPub)
	if err != nil {
		t.Fatal(err)
	}
